...
open class CircleSlider: UIControl {
  ...
  fileprivate var previousDegree = 0.0
  ...
  override open func layoutSublayers(of layer: CALayer) {
    ...
    if self.thumbView == nil {
        //modified by Michel
      self.thumbView = UIView(frame: CGRect(x: self.thumbWidth * 0.6, y: 0, width: self.thumbWidth * 3, height: self.thumbWidth))
        let transform : CGAffineTransform = CGAffineTransform.init(rotationAngle: CGFloat(135 * Float.pi / 180))
  ...
  override open func continueTracking(_ touch: UITouch, with event: UIEvent?) -> Bool {
    var degree = Math.pointPairToBearingDegrees(self.center, endPoint: touch.location(in: self))
    //modifiled by Michel
    if degree > 45 && degree < 135 {
        if degree - previousDegree > 0 {
            degree = 45
        } else {
            degree = 135.001
        }
  ...
  fileprivate func redraw() {
    ...
    self.thumbView = UIView(frame: CGRect(x: 0, y: 0, width: self.thumbWidth, height: self.thumbView.bounds.height))
  ...
  fileprivate func layout(_ degree: Double) {
    if let trackLayer = self.trackLayer, let thumbView = self.thumbView {
      ...
      thumbView.center = self.thumbCenter(degree)
        //modified by Michel Rabinger
        let rads = degree.degreesToRadians
  ...
  fileprivate func thumbCenter(_ degree: Double) -> CGPoint {
    let radius = (self.bounds.insetBy(dx: viewInset, dy: viewInset).width * 0.5) - (self.barWidth * 0.7)
